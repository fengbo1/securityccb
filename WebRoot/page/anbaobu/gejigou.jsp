...
function frame(o)
{
	...
	else if(obj=="address")//员工住址查询20180327。
	{
	...
	{
	   document.getElementById("frame").src="<%=path%>/record.action?type=find_all_desc&year=<%=yyyear%>&beginDate=&endDate=&timetb=time";
		}
			...
			<div class=x_anniu>
				<div>
					...
					<div class="b d1 k">
						<div class=x_gr>
							<!-- 登陆用户信息 -->
							<p style="padding:0px 0px 0px 0px;margin:0px 0px 0px 0px">${name }</p>
							<p style="padding:0px;margin:0px;font-size:12px;">（${jigouc}）</p>
							<p style="font-size:14px;padding:2px 0px 0px 0px;margin:0px 0px 0px 0px">欢迎访问<br/>安全管理与考核评价系统</p>
							<p style="padding:0px;margin:0px">
								<input align="right" name="logout" type="button" value=" 返   回 " onclick="location='<%=path%>/login.action?newnumber=${newnumber}&password=${password}'" />
								<input type="hidden" id="id" name="id" value="${id}"/>
							</p>
							<p style="font-size:14px;padding:0px 0px 0px 0px;margin:0px"><%=new SimpleDateFormat("yyyy年MM月dd日")	.format(new java.util.Date())%></p>
							<p id="txt" style="font-size:12px;padding:0px 0px 0px 0px;margin:0px 0px 0px 0px"></p>
							<p style="font-size:14px;padding:0px 0px 0px 0px;margin:0px 0px 0px 0px">支持电话：028-86027534</p>
